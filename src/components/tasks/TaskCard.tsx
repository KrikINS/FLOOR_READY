...
const TaskCard: React.FC<TaskCardProps> = ({ task }) => {
    ...
    return (
        <div className="bg-white border border-slate-200 rounded-lg shadow-sm hover:shadow-md transition-shadow p-4">
            <div className="flex justify-between items-start mb-2">
                <div className="flex-1">
                    <Link to={`/tasks/${task.id}`} className="text-lg font-medium text-slate-900 hover:text-primary block truncate">
                        {task.title}
                    </Link>
                    {task.events?.name && (
